// src/modules/manager/pages/website/FacebookPostsTab.jsx
import { useState, useEffect, useCallback } from "react";
import {
  getFacebookPosts, createFacebookPost, updateFacebookPost, deleteFacebookPost,
  activateFacebookPost, deactivateFacebookPost, reorderFacebookPosts, setFacebookPostFeatured,
  toAbsoluteMediaUrl,
} from "../../api/websiteApi";
import {
...
const EMPTY_FORM = {
  facebook_url: "", caption: "", is_active: true,
...
export default function FacebookPostsTab({ showToast }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getFacebookPosts({ search, ordering });
  ...
  const openEdit = (p) => {
    setEditing(p);
    setForm({
      facebook_url: p.facebook_url, caption: p.caption || "", is_active: !!p.is_active,
  ...
  const handleSave = async () => {
    if (!form.facebook_url.trim()) {
      setFormError("Facebook URL is required.");
      return;
    }
    setSaving(true);
    setFormError(null);
    try {
      const payload = { facebook_url: form.facebook_url, caption: form.caption, is_active: form.is_active };
      ...
      if (editing) {
        await updateFacebookPost(editing.id, payload);
        showToast("Post updated.");
      } else {
        await createFacebookPost(payload);
  ...
  const handleDelete = async () => {
    await deleteFacebookPost(confirmDelete.id);
  ...
  const toggleActive = async (p) => {
    ...
    try {
      if (p.is_active) await deactivateFacebookPost(p.id);
      else await activateFacebookPost(p.id);
  ...
  const toggleFeatured = async (p) => {
    ...
    try {
      ...
      await setFacebookPostFeatured(p.id, next);
  ...
  const move = async (index, direction) => {
    ...
    setItems(reordered);
    try {
      await reorderFacebookPosts(reordered.map((i) => i.id));
    } catch {
  ...
  return (
    <div>
      ...
      {loading ? <LoadingState /> : items.length === 0 ? (
        <EmptyState text="No Facebook posts added yet." />
      ) : (
      ...
      {showModal && (
        <Modal title={editing ? "Edit Post" : "Add Facebook Post"} onClose={() => setModal(false)}>
          <FormBanner message={formError} />
          <InputField label="Facebook URL" required hint="A post, video, or reel link from facebook.com or fb.watch.">
            <input style={inputStyle} value={form.facebook_url} onChange={(e) => setForm((f) => ({ ...f, facebook_url: e.target.value }))} placeholder="https://www.facebook.com/PageName/posts/..." />
          </InputField>
          ...
          <InputField label="Thumbnail">
            <PhotoUploadField
              ...
              hint="Optional, JPG or PNG — we don't scrape Facebook for a preview image, so this is shown on the card instead."
            />
          </InputField>
      ...
      {confirmDelete && (
        <ConfirmDialog
          ...
          message="Delete this Facebook post? This cannot be undone."

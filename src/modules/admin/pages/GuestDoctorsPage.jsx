// src/modules/admin/pages/GuestDoctorsPage.jsx
...
export default function GuestDoctorsPage() {
  ...
  const loadGuests = useCallback((url) => {
    ...
    getGuestDoctorList(url)
      ...
      .catch(() => setError("Failed to load guest doctors."))
      .finally(() => setLoading(false));
  }, []);

  useEffect(() => { loadGuests(buildUrl()); }, [buildUrl, loadGuests]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        // Create with empty placeholder name — reception fills it later
      ...
      if (modal === "add") {
        await createGuestDoctor(payload);
      } else {
        await patchGuestDoctor(editTarget.guest_doctor_id, payload);
      }

      setModal(null);
      showToast(modal === "add" ? "Guest doctor added!" : "Guest doctor updated!");
      loadGuests(currentUrl ?? buildUrl());
    } catch (err) {
  ...
  const handleToggleStatus = async (g) => {
    if (!window.confirm(`${g.is_active ? "Deactivate" : "Reactivate"} ${g.guest_code}?`)) return;
    try {
      if (g.is_active) await deactivateGuestDoctor(g.guest_doctor_id);
      else             await reactivateGuestDoctor(g.guest_doctor_id);
      loadGuests(currentUrl ?? buildUrl());
    } catch {
  ...
  return (
    <div style={{ background: "#F8FAFC", minHeight: "100%", margin: "-20px -16px", padding: "24px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
        <div>
          <h1 style={{ fontSize: "20px", fontWeight: 700, color: "#0F172A" }}>Guest Doctors</h1>
        ...
        <button onClick={openAdd}
          ...
          <Ico path={ICONS.plus} size={15} color="#fff" /> Add Guest Doctor
        </button>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: "14px", padding: "16px", boxShadow: "0 1px 3px rgba(0,0,0,0.07)", border: "1px solid #F1F5F9", marginBottom: "16px", display: "flex", gap: "12px", flexWrap: "wrap", alignItems: "center" }}>
        ...
        <span style={{ fontSize: "12px", color: "#94A3B8", marginLeft: "auto" }}>{guests.length} of {count} shown</span>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: "14px", boxShadow: "0 1px 3px rgba(0,0,0,0.07)", border: "1px solid #F1F5F9", overflow: "hidden" }}>
              ...
              {loading ? (
                ...
                guests.map(g => (
                  ...
                  >
                    ...
                    <td style={{ padding: "12px 14px" }}><StatusDot active={g.is_active} /></td>
                    <td style={{ padding: "12px 14px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                        <button onClick={() => openEdit(g)} title="Edit credentials"
                        ...
                        <button onClick={() => handleToggleStatus(g)}
                          title={g.is_active ? "Deactivate" : "Reactivate"}
                          style={{ background: g.is_active ? "#FEF2F2" : "#F0FDF4", border: "none", borderRadius: "7px", padding: "6px", cursor: "pointer", display: "flex" }}>
                          <Ico path={g.is_active ? ICONS.deact : ICONS.react} size={14} color={g.is_active ? "#EF4444" : "#16A34A"} />
                        </button>
                      </div>
                    </td>
                  </tr>
        ...
        {(next || prev) && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderTop: "1px solid #F1F5F9" }}>
            <button onClick={() => prev && loadGuests(prev)} disabled={!prev}
            ...
            <button onClick={() => next && loadGuests(next)} disabled={!next}
      ...
      {modal && (
        <Modal
          title={modal === "add" ? "Add Guest Doctor" : `Edit Credentials — ${editTarget?.guest_code}`}
          subtitle={
            modal === "add"
              ? "Only login credentials are needed. Reception will set the name and fee when booking."
              : "Update the login credentials for this guest doctor account."
          }

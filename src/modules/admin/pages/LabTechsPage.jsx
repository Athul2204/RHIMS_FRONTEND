// src/modules/admin/pages/LabTechsPage.jsx
...
const ICONS = {
  ...
  lab:    "M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z M13 2v7h7",
};
...
export default function LabTechsPage() {
  ...
  const load = useCallback((arg = null) => {
    setLoading(true);
    getStaffList(arg || buildUrl())
      .then(d => {
        setList(d.results ?? []);
        setCount(d.count ?? 0);
        setNext(d.next);
        setPrev(d.previous);
        setCurrentUrl(arg || buildUrl());
      })
      .catch(() => setError("Failed to load lab technicians."))
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setModal(null);
      showToast(modal === "add" ? "Lab Technician added!" : "Lab Technician updated!");
      load(currentUrl);
    } catch (err) {
      console.error("LabTech form error:", err);
  ...
  return (
    <div style={{ background: "#F8FAFC", minHeight: "100%", margin: "-20px -16px", padding: "24px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
        <div>
          <h1 style={{ fontSize: "20px", fontWeight: 700, color: "#0F172A" }}>Lab Technicians</h1>
        ...
        <button onClick={openAdd} style={{ display: "flex", alignItems: "center", gap: "7px", background: G, color: "#fff", border: "none", borderRadius: "10px", padding: "10px 18px", fontSize: "13px", fontWeight: 600, cursor: "pointer" }}>
          <Ico path={ICONS.plus} size={15} color="#fff" /> Add Lab Tech
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: "#fff", borderRadius: "14px", padding: "64px", textAlign: "center", border: "1px solid #F1F5F9" }}>
          <p style={{ color: "#94A3B8" }}>No lab technicians found.</p>
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: "16px" }}>
          {displayed.map(s => {
            return (
              <div key={s.id} style={{ background: "#fff", borderRadius: "14px", padding: "20px", border: `1px solid ${s.is_active ? "#F1F5F9" : "#FEE2E2"}`, boxShadow: "0 1px 3px rgba(0,0,0,0.07)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "14px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    <div style={{ width: "44px", height: "44px", borderRadius: "12px", background: "#FDE68A", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px", fontWeight: 700, color: G }}>
                      {(s.user?.first_name?.[0] ?? "L").toUpperCase()}
                    </div>
                    <div>
                  ...
                  <span style={{ fontSize: "11px", padding: "3px 9px", borderRadius: "20px", background: s.is_active ? "#FDE68A" : "#FEE2E2", color: s.is_active ? "#B45309" : "#DC2626", fontWeight: 600 }}>
                ...
                <div style={{ display: "flex", gap: "8px", borderTop: "1px solid #F8FAFC", paddingTop: "12px" }}>
                  <button onClick={() => openEdit(s)} style={{ flex: 1, padding: "7px", borderRadius: "8px", border: `1px solid #FDE68A`, background: "#FFFBEB", cursor: "pointer", fontSize: "12px", fontWeight: 500, color: G }}>Edit</button>
      ...
      {modal && (
        <Modal title={modal === "add" ? "Add New Lab Tech" : `Edit — ${editTarget?.staff_code}`} onClose={() => setModal(null)}>
          ...
          <div style={{ maxHeight: "70vh", overflowY: "auto", paddingRight: "8px" }}>
             <div style={{ background: "#FFFBEB", border: "1px solid #FDE68A", borderRadius: "8px", padding: "10px 14px", marginBottom: "14px", fontSize: "12px", color: "#92400E" }}>
              Lab Technicians are only given login credentials to mark attendance and record work — the hospital does not process their salary through this system, so no salary field is collected here.
            </div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px", marginTop: "20px", paddingTop: "16px", borderTop: "1px solid #F1F5F9" }}>
            ...
            <button onClick={handleSubmit} disabled={submitting}
              style={{ padding: "9px 22px", borderRadius: "9px", border: "none", background: submitting ? "#FCD34D" : G, cursor: "pointer", fontSize: "13px", fontWeight: 600, color: "#fff" }}>
              {submitting ? "Saving…" : modal === "add" ? "Add Lab Tech" : "Save Changes"}
            </button>
          </div>
        </Modal>
      )}

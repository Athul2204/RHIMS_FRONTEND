// src/modules/pharmacist/pages/PrintBillPage.jsx
...
// Builds a "1 dose · Twice Daily · After Meals · 5 days" style summary from
// the prescription context the backend attaches to each bill medicine item.
// Returns null for items with no linked prescription (e.g. walk-in sales).
  ...
  return (
    <div style={{ fontFamily:"'Inter',sans-serif", maxWidth:"148mm", margin:"0 auto", padding:"16px 16px 40px" }}>
      ...
      <div className="no-print" style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        ...
        <button onClick={handlePrint} style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 18px", borderRadius:8, border:"none", background:G, color:"#fff", cursor:"pointer", fontSize:13, fontWeight:700 }}>
      ...
      <div ref={printRef} id="bill-print" style={{ background:"#fff", border:"1px solid #E5E7EB", borderRadius:12, padding:"24px", boxShadow:"0 4px 20px rgba(0,0,0,0.05)", boxSizing:"border-box" }}>
        ...
        {/* Status badge */}
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:20, padding:"8px 14px", background: bill.bill_status === "PAID" ? "#F0FDF4" : "#FFF7ED", borderRadius:8, border:`1px solid ${bill.bill_status === "PAID" ? "#BBF7D0" : "#FED7AA"}` }}>
          <Ico d={ICONS.check} size={14} color={bill.bill_status === "PAID" ? "#15803D" : "#C2410C"} />
          <span style={{ fontSize:12, fontWeight:700, color: bill.bill_status === "PAID" ? "#166534" : "#C2410C" }}>
            {bill.bill_status === "PAID" ? "Paid — medicines dispensed" : bill.bill_status}
          </span>
        </div>
                      ...
                      {(dosage || item.instructions) && (
                        <tr style={{ borderBottom: isLast ? "none" : "1px solid #F8FAFC" }}>
                          <td colSpan={4} style={{ padding:"0 10px 8px" }}>
                            {dosage && (
                              <p style={{ margin:0, fontSize:11, fontWeight:600, color:"#6D28D9" }}>{dosage}</p>
                            )}
                            {item.instructions && (
